...
- **A theme of my own**: cream `#EEE9DF`, slate `#2C3B4D`, amber `#FFB162`. Body text passes WCAG AAA against the cream; the amber is reserved for fills and borders, with a deeper amber for link text so the brand colour doesn't fail contrast.
- **Logo and favicon** wired up. Header centred on a 3-column grid so the nav stays dead-centre regardless of what flanks it; a burger menu kicks in below 720px.
...
- A domain (something around £10/year) so the URL stops including `/field-notes`.
